...
m1=1
curve(((4*N*(1-x)*x*N)/(((1-x)*N*(1+f))+(x*N*(1+m1))))/N, from=0, to=1, add=T)
...
curve(((9*N*(1-x)*N*(x))/((4*N*(1-x)*(1+f))+(2*N*(x)*(1+m1))))/N, from=0, to=1, add=T)
...
curve(((9*Ne_2*x*(x-1)/(2*(x-2))))/(4*N*(1-x)*x), from=0, to=1, add=T)
...
curve(Ne_2*(1-x)/(2)/(4*N*(1-x)*x),from=0,to=1, add=T) #Ychromosomes
