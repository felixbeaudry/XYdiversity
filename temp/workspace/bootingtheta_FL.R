...
#x-linked genes
flx$weighted_theta <- txx$SynSitestot * txx$theta_syntot            
flx_ave_weighted_theta=(sum(txx$weighted_theta))/(sum(txx$SynSitestot))

#data for x linked genes
a=flx$SynSitestot #stores the number of synonymous sites for X-linked genes in vector
b=flx$theta_syntot #stores theta synonymous for X-linked genes in vector
...
lowerXA_FL=0.0014/tf_all_ave_weighted_theta
upperXA_FL=0.0021/tf_all_ave_weighted_theta
